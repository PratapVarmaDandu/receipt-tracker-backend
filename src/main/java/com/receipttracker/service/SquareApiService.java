...
public class SquareApiService {
    ...
    /** Immutable credential bundle sourced exclusively from per-org config. */
    public record SquareCreds(String accessToken, String environment,
                              String applicationId, String locationId) {
        String baseUrl() {
    ...
    private HttpHeaders authHeaders(SquareCreds creds) {
    ...
    public List<SquareCatalogItemDTO> getCatalogItemsForCreds(SquareCreds creds, String cursor) {
        try {
            ...
            HttpEntity<Map<String, Object>> req = new HttpEntity<>(body, authHeaders(creds));
            ResponseEntity<CatalogSearchResponse> resp = restTemplate.exchange(
                    creds.baseUrl() + "/catalog/search", HttpMethod.POST, req, CatalogSearchResponse.class);
    ...
    public List<Map<String, String>> getCategoriesForCreds(SquareCreds creds) {
        try {
            HttpEntity<Void> req = new HttpEntity<>(authHeaders(creds));
            ResponseEntity<CatalogListResponse> resp = restTemplate.exchange(
                    creds.baseUrl() + "/catalog/list?types=CATEGORY", HttpMethod.GET, req, CatalogListResponse.class);
            ...
            return data.objects.stream()
                    ...
                    .map(o -> { Map<String, String> m = new LinkedHashMap<>(); m.put("id", o.id); m.put("name", o.categoryData.name); return m; })
                    .collect(Collectors.toList());
    ...
    public List<Map<String, Object>> getLocationsForCreds(SquareCreds creds) {
        try {
            HttpEntity<Void> req = new HttpEntity<>(authHeaders(creds));
            ResponseEntity<AllLocationsResponse> resp = restTemplate.exchange(
                    creds.baseUrl() + "/locations", HttpMethod.GET, req, AllLocationsResponse.class);
    ...
    public String getLocationNameForCreds(SquareCreds creds, String forLocationId) {
        try {
            String id = (forLocationId != null && !forLocationId.isBlank())
                    ? forLocationId : creds.locationId();
            HttpEntity<Void> req = new HttpEntity<>(authHeaders(creds));
            ResponseEntity<SingleLocationResponse> resp = restTemplate.exchange(
                    creds.baseUrl() + "/locations/" + id, HttpMethod.GET, req, SingleLocationResponse.class);
    ...
    /** Returns true if the credentials can reach the Square Locations API. */
    public boolean testConnectionForCreds(SquareCreds creds) {
        try {
            List<Map<String, Object>> locations = getLocationsForCreds(creds);
            return !locations.isEmpty() || true; // empty locations is still a valid account
        } catch (Exception e) {
            return false;
        }
    }
    ...
    public Map<String, Object> createOrderAndChargeForCreds(SquareCreds creds,
                                                             CreateSquareOrderRequest request) {
        String resolvedLocationId = (request.getLocationId() != null && !request.getLocationId().isBlank())
                ? request.getLocationId() : creds.locationId();
        ...
        HttpEntity<Map<String, Object>> orderReq = new HttpEntity<>(orderBody, authHeaders(creds));
        ResponseEntity<OrderCreateResponse> orderResp = restTemplate.exchange(
                creds.baseUrl() + "/orders", HttpMethod.POST, orderReq, OrderCreateResponse.class);
        ...
        HttpEntity<Map<String, Object>> payReq = new HttpEntity<>(payBody, authHeaders(creds));
        ResponseEntity<SquarePaymentResponse> payResp = restTemplate.exchange(
                creds.baseUrl() + "/payments", HttpMethod.POST, payReq, SquarePaymentResponse.class);
    ...
    private SquareCatalogItemDTO toItemDTO(CatalogObject obj,
        ...
        if (d.variations != null) {
            for (CatalogObject v : d.variations) {
                ...
                variations.add(new SquareVariationDTO(v.id, v.itemVariationData.name, price,
                        v.itemVariationData.priceMoney != null ? v.itemVariationData.priceMoney.currency : "USD",
                        true));
            }
        }

        return new SquareCatalogItemDTO(obj.id, d.name, d.description, d.categoryId, catName, imageUrl, variations);
    }
    ...
    @JsonIgnoreProperties(ignoreUnknown = true)
    static class ImageData { @JsonProperty("url") String url; }

    @JsonIgnoreProperties(ignoreUnknown = true)
    static class CategoryData { @JsonProperty("name") String name; }
